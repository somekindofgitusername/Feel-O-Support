...
Privacy Policy for Time-O
This Privacy Policy describes how Time-O ("the App") collects, uses, and discloses information when you use our mobile application.
...
Location Data: To provide accurate time calculations for certain time domains (e.g., Apparent Solar Time, Local Sidereal Time, Zmanim, Wadokei, Roman Horae) that are dependent on geographic position, the App may request access to your device's location.
This location data is collected only when the App is in use ("When In Use" authorization).
We request a low accuracy level (kilometer accuracy) and filter for significant movement (1km) to minimize power consumption and data precision.
You have the option to manually input a specific latitude and longitude within the App's settings, overriding the device's location services.
